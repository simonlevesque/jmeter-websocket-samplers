...
public class WebSocketClient {
  ...
  public HttpResult connect(Map<String, String> headers, int connectTimeout, int readTimeout) throws IOException, HttpException {
    ...
    if (connectUrl.getPort() == -1){
      ...
      portStr = ":" + connectUrl.getPort();
      port = connectUrl.getPort();
    }

    boolean connected = false;
    log.debug("Creating connection with " + connectUrl.getHost() + ":" + port);
    ...
    wsSocket = createSocket(connectUrl.getHost(), port, connectTimeout, readTimeout);
    ...
    try {
      ...
      String hostLine = "Host: " + connectUrl.getHost() + portStr;
      httpWriter.print(hostLine + NEW_LINE);
      log.debug(">> " + hostLine);
